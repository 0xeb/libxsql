...
// Query handler that uses global db
...
TEST_F(ThinclientTest, ServerStatus) {
    server_config config;
    config.port = 18083;
    config.on_query = [](const std::string&) -> std::string { return ""; };
    config.on_status = []() -> std::string { return "{\"status\": \"test\"}"; };
